...
def create_copies(file_paths, target_dir):
    """Copy a list of file paths to the target directory."""
...
def main():
    ...
    for folder_name, file_list in dirs_to_link:
        print(f"Copying {len(file_list)} files -> {folder_name}")
        create_copies(file_list, os.path.join(dataset_out_dir, folder_name))
